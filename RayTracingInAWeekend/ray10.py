...
lower_left_corner = ti.Vector(lookfrom - half_width * u - half_height * v - w)
horizontal = ti.Vector(u * 2 * half_width)
vertical = ti.Vector(v * 2 * half_height)
...
def paint():
    for i, j in pixels:
        ...
        for _ in ti.static(range(4)):
            ...
            direction = lower_left_corner + uu * horizontal + vv * vertical - origin
            col += color(origin, unit_vector(direction))
        pixels[i, j] = col * 0.25


gui = ti.GUI("Ray10", (nx, ny))
for i in range(1000):
    paint()
